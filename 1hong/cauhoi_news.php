...
if(isset($_POST['ok']))
{	
//Lấy số lượng câu  hỏi của đề update vào đề thi
	$sql_slcauhoi="SELECT * FROM xh_cauhoi WHERE made=".$_POST['ganmade']." ";
	$slch=mysql_query($sql_slcauhoi);
	$demslcauhoi=mysql_num_rows($slch)+1;
			$sqlslch="UPDATE xh_dethi SET 
			socau = '".$demslcauhoi."'   
			WHERE made = ".$_POST['ganmade']."";
			mysql_query($sqlslch);	
//end cập nhật SL câu hỏi

			/* Lấy thêm tên đề thi trong xh_dethi chèn vào bảng câu hỏi xh_cauhoi*/
			$sql="select * from xh_dethi where made like '$_POST[ganmade]'";
	...
	//Loại bỏ các kí tự HTML, xóa hết khoảng trắng trước khi thêm
	 $sqla="INSERT INTO xh_cauhoi(made,loaide,mon,khoi,noidung,huongdan,dapan,diem, duyetde,mucdo)  
		VALUES ('$_POST[ganmade]','$ganloaide','$ganmon','$gankhoi','$_POST[noidung]','$_POST[huongdan]','$dapan','$_POST[diem]','$_POST[duyetde]','$_POST[mucdocauhoi]')";
